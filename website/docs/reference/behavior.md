...
### 대상
- 백틱으로 감싼 텍스트 중 `Class.`, `Datatype.`, `Enum.`, `Global.`, `Library.` 접두어는 링크로 변환됩니다.
- Bakerywave 내부 레퍼런스는 `Classes.`, `Docs.` 접두어를 사용합니다.

### 변환 규칙
- `` `Class.Name` `` → 해당 클래스 페이지로 연결
- `` `Class.Name:Method()` `` → 클래스 페이지의 해당 멤버 앵커로 연결
- `` `Class.Name.Property` `` → 클래스 페이지의 해당 멤버 앵커로 연결
- `` `Docs.Features.DocFeatureClass` `` → Bakerywave 레퍼런스 페이지로 연결
- `` `Docs.Features.DocFeatureClass:Double()` `` → 해당 멤버 앵커로 연결
...
- `Classes.`, `Docs.`는 Bakerywave 내부 레퍼런스로 연결됩니다.
